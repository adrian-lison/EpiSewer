...
#' @param gamma_scale Scale parameter of the Gamma distribution. Can be specified instead of the rate. Only has an effect if the rate is not specified.
...
# This is adapted from EpiEstim, credit to Anne Cori a.cori@imperial.ac.uk
#' Title
#'
#' @param gamma_mean
#' @param gamma_sd
...
#' @export
get_discrete_lognormal <- function(meanlog, sdlog, unit_mean = NULL, unit_sd = NULL, maxX, include_zero = T, print_params = F) {
  ...
  if (include_zero) {
    ...
    probs <- c(
      shortest,
      sapply(2:(maxX - 1), function(x) plnorm(x + 1, meanlog = meanlog, sdlog = sdlog) - plnorm(x, meanlog = meanlog, sdlog = sdlog)), # all other (discrete)
      longest
    )
  }
