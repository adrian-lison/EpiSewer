...
real dPCR_conc_lpdf(real y, real lambda, real m, real c, int norm_l, int norm_u) {
...
real dPCR_conc_lpdf(vector y, vector lambda, vector m, real c, int norm_l, int norm_u) {
  int N = num_elements(y);
  real lp = 0;
  for (i in 1:N) {
    lp += dPCR_conc_lpdf(y[i] | lambda[i], m[i], c, norm_l, norm_u);
  }
  return lp;
}

// vectorized version without normalization
real dPCR_conc_lpdf(vector y, vector lambda, vector m, real c) {
  int N = num_elements(y);
...
vector dPCR_conc_rng(vector lambda, array[] int m, real c) {
  int N = num_elements(lambda);
  vector[N] p = 1 - exp(-lambda * c);
...
real dPCR_conc_nonzero_lpdf(vector y, vector lambda, vector m, real c) {
  int N = num_elements(y);
  vector[N] p = 1 - exp(-lambda * c);
  real obs_mass = binom_approx_lpdf(m .* (1 - exp(-y * c)) | m, p);
  // normalize
