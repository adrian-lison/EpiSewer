...
We here estimate it using a Gaussian process prior (`R_estimate_gp()`), but 
there are other smoothing approaches available (see `component_functions("R")`).
...
ww_forecast2 <- model_forecast(
  ...
  damping = damping_assume(0.8)
)
```

The `damping` component allows to dampen the forecast of Rt so that trends in transmission will level off after some time. This prevents unrealistic extrapolation of transmission dynamics. The default is `0.8`, meaning that the trend is reduced by 20% each day until the Rt forecast is essentially constant.
