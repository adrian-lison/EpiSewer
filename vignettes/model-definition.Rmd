...
This option models $R_t$ as a Gaussian process (GP) with a Matérn kernel, i.e.

\begin{equation}
\text{link}(R_t) = 1 + \delta_t\quad |\quad (1-\phi L)\delta \sim \mathcal{GP}\,\!\big(0,\, k_\nu(\tau;\,\ell_R,\sigma^2_R)\big),
\end{equation}

where $\nu$ is the kernel smoothing parameter, $\ell_R$ the length scale, and
$\sigma^2_R$ the marginal variance of the GP, and $L$ is the lag operator. The
parameter $\phi$ is a damping factor that controls the autocorrelation of the
GP. When $\phi=1$, the Gaussian process effectively applies to the first order
differences of $R_t$. This is desirable for real-time estimation, as it encodes
the minimally informed prior assumption that the current transmission dynamics
will continue unchanged. However the resulting non-stationarity of the time
series slows down MCMC sampling. For $\phi<1$, we get an AR(1) process that is
stationary and reverts to $R_t=1$ in the long term. We here use a default of
$\phi=0.95$, which samples faster than $\phi=1$ due to stationarity but still
predicts continued transmission dynamics on short-term scales.
...
This option enforces a smooth first derivative of $R_t$ by placing cubic splines on the first-order differences of $R_t$, \ie
...
Options 2-4 also support changes in the variability of $R_t$ over time.
