# Partial compile with the components that flows_observe depends on
compile_with_flows <- function(flows_component) {
  modeldata_compile(list(
    concentrations_observe(
      measurements = ww_data_SARS_CoV_2_Zurich$measurements[date <= "2022-04-07",]
    ),
    horizon_assume(horizon = 7),
    flows_component
  ))
}
...
test_that("flows_observe() throws error when flow is missing on sampled dates", {
  ...
  expect_error(
    compile_with_flows(flows_observe(flows = flow_missing)),
...
test_that("flows_observe() imputes flow values missing for unsampled dates", {
  ...
  flow_missing <- ww_data_SARS_CoV_2_Zurich$flows[!(date %in% missing_dates),]
  expect_message(
    md_flow <- compile_with_flows(flows_observe(flows = flow_missing)),
